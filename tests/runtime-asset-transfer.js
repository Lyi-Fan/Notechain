...
const open=async id=>{qa.navigate('/cases/'+caseId+'/assets/'+id);await wait(()=>document.querySelector('[data-asset-id="'+id+'"] .tiptap')?.editor?.state.doc.textContent.includes(id===sourceId?'中文笔记':'目标笔记正文。'),'Asset body not ready');await document.fonts.ready;for(let frame=0;frame<4;frame++)await new Promise(requestAnimationFrame);return document.querySelector('[data-asset-id="'+id+'"] .tiptap').editor};
...
editor.view.focus();editor.commands.setTextSelection(range);await wait(()=>document.querySelector('[aria-label=加入中转站]'),'Source selection toolbar missing');document.querySelector('[aria-label=加入中转站]').click();
let originalHref;
for(const [index,id]of targets.entries()){
  ...
  if(index===0){
    editor.view.focus();let selection;editor.state.doc.descendants((node,pos)=>{if(node.marks.some(mark=>mark.type.name==='link'&&mark.attrs.href===href))selection={from:pos,to:pos+node.nodeSize}});
