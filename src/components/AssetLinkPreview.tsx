...
interface Props { root: RefObject<HTMLElement>; assets: AssetRecord[]; cases: CaseRecord[]; findings: FindingRecord[]; tasks: TaskRecord[]; onOpen: (href: string) => void; renderEditor: (item: PreviewItem) => ReactNode }
export interface AssetLinkPreviewHandle { dismiss: () => void }
const DURATION = 220
...
export const AssetLinkPreview = forwardRef<AssetLinkPreviewHandle, Props>(function AssetLinkPreview({ root, assets, cases, findings, tasks, onOpen, renderEditor }, ref) {
  ...
  const previewReady = useNoteReady(previewKind === 'asset' ? 'assets' : previewKind === 'finding' ? 'findings' : 'cases', view?.id.slice((previewKind?.length ?? 0) + 1) ?? '')
  ...
  const find = (href: string, byKey = false) => findPreviewItem(href, assets, cases, findings, caseNames, byKey)
  ...
  useEffect(() => { if (current.current && !current.current.webTitle && !data.current.find(current.current.id, true)) update(null) }, [assets, cases, findings, caseNames])
  useEffect(() => {
    if (!modal) return
  ...
  const item = useMemo(() => (view && !view.webTitle ? findPreviewItem(view.id, assets, cases, findings, caseNames, true) : undefined) ?? (view?.webTitle ? { kind: 'web' as const, title: view.webTitle, subtitle: new URL(view.href).hostname } : undefined), [assets, cases, findings, caseNames, view?.id, view?.webTitle, view?.href])
  ...
  return createPortal(<div className={`notebook-preview-layer${modal ? ' is-modal' : ''}`} onPointerDown={(event) => { if (event.target === event.currentTarget) { event.preventDefault(); close() } }}>
    <section ref={panel} className="notebook-link-preview" data-mode={view.mode} role={modal ? 'dialog' : 'region'} aria-label={'\u8d44\u4ea7\u9884\u89c8'} aria-modal={modal || undefined} style={box} onPointerDown={(event) => { const target = event.target as HTMLElement; if (modal && !target.closest('[contenteditable="true"], input, textarea, select, button, a, .cm-editor, .notebook-bubble, .notebook-picker')) reader.current?.focus({ preventScroll: true }) }} onKeyDown={(event) => {
      ...
      <header className="notebook-preview-head"><div><small>{item.subtitle}</small><strong>{item.title}</strong></div><button type="button" className="icon-button" aria-label={'\u6253\u5f00\u5173\u8054\u7b14\u8bb0'} title={'\u6253\u5f00\u5173\u8054\u7b14\u8bb0'} onClick={() => navigate(view.href)}><ArrowUpRight size={17} /></button><button type="button" className="icon-button" aria-label={modal ? '\u5173\u95ed\u9884\u89c8' : '\u5c55\u5f00\u9884\u89c8'} title={modal ? '\u5173\u95ed\u9884\u89c8' : '\u5c55\u5f00\u9884\u89c8'} onClick={modal ? close : expand}>{modal ? <X size={17} /> : <Maximize2 size={16} />}</button></header>
      <div ref={reader} className="notebook-preview-reader" tabIndex={modal ? 0 : -1}>{item.kind === 'web' ? <div className="markdown-body"><p style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{view.excerpt || displaySourceUrl(view.href)}</p></div> : !previewReady ? <p role="status">正在读取笔记…</p> : modal ? renderEditor(item) : <MarkdownView content={content} onInternalLink={navigate} />}</div>
    </section>
  </div>, document.body)
})
